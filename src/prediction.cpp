...
// Replayable per-step LSTM graph: built once, replayed each step. Inputs (registration
// order): x0, then per layer [h_in, c_in]; all sliced from the coalesced input buffer.
// Captures per layer [c_out, h_out] land in the stable cap_* buffers.
...
// Replayable batched LSTM graph, one per batch size N, replayed each round. The
// per-layer captures land in stable internal buffers (out_state is reassigned
// each round) and are copied out after compute.
...
void PredictionNet::step(int32_t token_id, bool is_sos,
    ...
    // On GPU the per-step LSTM is launch-overhead bound: replay one captured
    // graph instead of launching each op directly. On CPU the work is cheap and
    // replay's set_input + readback would regress, so CPU keeps run_graph. Both
    // paths are byte-identical.
    //
    // Capture dsts live inside StepReplay (stable across steps); out_state is
    // .assign()'d every step and can move its data pointer, so we capture
    // internally and copy out after compute.
    ...
    // Coalesce per-step inputs into one host buffer uploaded with a single
    // set_input; the graph slices it into x0 / h_in[l] / c_in[l] views.
    if (!replay_) {
...
void PredictionNet::step_batch(const std::vector<int32_t>& token_ids,
    ...
    // One captured graph per N. Captures land in stable internal buffers
    // (out_state is reassigned each round) and are copied out after compute.
